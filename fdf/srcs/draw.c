...
/*   Updated: 2021/08/16 21:56:29 by hkubo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_map(t_info *info)
{
	...
	while (i < info->row_count - 1)
	{
		...
		while (j < info->col_count[i])
		{
			...
			y_convert = convert_y(info->map[i][j].x * info->ratio * 0.7, info->map[i][j].y * info->ratio * 0.7, info->map[i][j].z * 5, 30.0);
			// y_convert = convert_y(info->map[i][j].x * info->ratio * 0.7, info->map[i][j].y * info->ratio * 0.7, 0, 30.0);
			printf("i: %d j: %d x: %d y: %d ratio: %d\n", i, j, x_convert, y_convert, info->ratio);
