...
/*   Updated: 2021/08/15 22:08:15 by hkubo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate(t_info *info)
{
	...
	while (i < info->row_count - 1)
	{
		...
		while (j < info->col_count[i])
		{
			x_convert = convert_x(info->map[i][j].x * info->ratio, info->map[i][j].y * info->ratio, 30.0);
			// y_convert = convert_y(info->map[i][j].x * 10, info->map[i][j].y * 10, info->map[i][j].z / 10, 30.0);
			y_convert = convert_y(info->map[i][j].x * info->ratio, info->map[i][j].y * info->ratio, 0, 30.0);
