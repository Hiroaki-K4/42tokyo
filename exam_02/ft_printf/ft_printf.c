...
/*   Updated: 2021/11/02 10:32:27 by hkubo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int no_field_int(t_plist flag_list, char *str_num, int num, int keta)
{
	int len;
	char *tmp;
...
int print_string(va_list *ap, t_plist flag_list)
{
	...
	if (flag_list.field != -1)
	{
		if (flag_list.precision != -1 && flag_list.format < (int)ft_strlen(str))
		{
			i = flag_list.precision;
			while (flag_list.field - (i++) > 0)
				write(1, " ", 1);
